...
$di->setShared('voltService', function ($view, $di) use ($config) {
    ...
    $compiler
        ...
        ->addFunction('meta', '\Tag\AppTag::meta')

        ->addFunction('highlightHostByZone', '\Tag\AppTag::highlightHostByZone')

        ->addFunction('widgets', function () use ($di) {
            return '$this->widgetsTag->render()';
        })

        ->addFunction('favicon', '\Tag\AppTag::favicon')
        ->addFunction('meta_no_index_no_follow', '\Tag\AppTag::metaNoIndexNoFollow')
        ->addFunction('t', function ($resolvedArgs, $exprArgs) use ($compiler) {
            $key = $compiler->expression($exprArgs[0]['expr']);
            $placeholders = !isset($exprArgs[1]) ? 'null' : $compiler->expression($exprArgs[1]['expr']);
            return sprintf('$this->translator->_(%s, %s)', $key, $placeholders);
        })
